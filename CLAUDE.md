...
**작업 재개:** 새 세션에서 이 프로젝트를 이어받을 때는 `_workspace/HANDOFF.md` 를 먼저 읽어라. 현재 상태·다음 할 일·되돌리면 안 되는 설계 결정이 정리돼 있다.
...
| 2026-09-21 | 외부 UI 린터(impeccable 0.1.5) 평가 — 저장소 미포함 결정 | .gitignore, eslint.config.mjs | 신호비 8%(267건 중 21건). `cramped-padding` 246건은 Tailwind v4 가 간격을 `calc(var(--spacing)*N)` 로 뽑는 걸 정적 파서가 못 풀어 생기는 구조적 오탐 — A/B 로 확정(`padding-top:32px` 통과 / `calc(var())` 적발). 도구는 로컬에만 두고 훅은 해제 |
| 2026-09-21 | `.claude/` 를 차단 목록 → **화이트리스트**로 전환 | .gitignore | `publish-gate` 가 검사 첫 줄에서 `git add -A` 를 돌린다. 규정대로 게이트를 돌리는 순간 14.0 MiB exe 를 포함한 52파일이 스테이징됐다. 차단 목록은 `ops/*.py` 3회 누락과 같은 실수 — 외부 설치물은 디렉토리 이름을 예고하지 않는다. 기존 에이전트 6 + 스킬 5 만 `!` 로 연다 |
| 2026-09-21 | 게이트에 바이너리·서드파티 설치물 검사 신설 | skills/publish-gate | 1~4번 검사가 전부 텍스트 grep 이라 14 MiB PE 바이너리가 **무히트로 통과**했다. 통과가 아니라 검사 대상이 아니었던 것. 실행 파일·1MB 초과는 `NEEDS_USER_DECISION` |
| 2026-09-21 | 상세·About 의 h1 위 마이크로 라벨 제거 | components/ProjectDetail.tsx, AboutPage.tsx | 자간 넓힌 대문자 라벨 + 디스플레이 h1 조합은 현재 AI 생성 히어로의 기본값. 상세는 바로 위 `BackLink` 가 같은 그룹 라벨을 이미 찍어 정보 손실 0, About 은 `<title>`·nav 와 3중 중복. 홈 히어로엔 원래 없었다 — 라벨 없는 히어로가 이미 사이트 표준이었다. 카드(`ProjectCard`)는 돌아갈 길이 없어 유지 |
| 2026-09-21 | 영문 홈 산문의 em-dash 8개를 콜론·문장분할로 교체 | content/copy/en/home.md | 국문 산문은 em-dash 0개인데 영문만 절 접속자로 8번 썼다. 1차 독자가 영어권인데 **영문판에만 있는 리듬**이라는 비대칭이 근거. 데이터 층(`highlights`)은 열지 않았다 — 실측 결과 17건 중 9건이 ko/en 대칭이라 en 만 고치면 없던 비대칭을 새로 만든다. EN 전용 4건(akds 2 · training-coach 1 · financial-manager 1)은 다음 JSON 갱신 때 처리 |
| 2026-09-22 | 전면 재디자인 — 숫자 스트립 폐지, 하네스 도식 신설, 카피 전면 교체 | app/ components/ content/ lib/ | 사용자 피드백 "너무 밋밋해 글자만 있고 심심해", "2years monthly session 이런 건 무슨 말인지도 모를 거다. 데이터를 보여주는 게 목적이 아니라 그걸로 공감을 끌어내야지". 에이전트 3인 회의(아이디어→반박→합의). 근거는 `_workspace/01_architect_ia.md` 재디자인 절 |
| 2026-09-22 | PROOF 스트립(6·5·2·2년) 폐지 | components/HomePage.tsx, content/copy/*/home.md | 커밋 메시지 40건 전체에 **단독으로 선 숫자가 0개**인데 사이트만 카운터 4칸을 세워 뒀다. 사용자 본인의 규칙을 사이트가 어긴 자리였다. 옳은 건 값이지 칸이 아니므로, AKDS 연속성 1건만 역할 목록 옆으로 옮기고 나머지는 뒤 섹션이 직접 센다 |
| 2026-09-22 | 하네스 도식 신설 — 게이트를 **끊긴 경로**로 그린다 | components/HarnessFlow.tsx | 장식 금지(`00_purpose.md`)는 유지하되 통과 기준을 세웠다: **이 그래픽이 JSON의 어느 필드를 읽는가에 답하지 못하면 넣지 않는다.** 검수 기준 = 모션 끈 캡처에서 "어디서 멈추는가"에 답할 수 없으면 실패. GATE 이후 선을 그리지 않아 정지 상태에서도 차단이 읽힌다 |
| 2026-09-22 | 도식 캡션을 "6개 전부"가 아니라 "3개"로 | components/HarnessFlow.tsx | 설계 단계의 성격 규정("여섯 시스템 전부에 멈출 수 있는 역할이 하나씩")이 데이터와 어긋났다. `members[].role` 에 실제로 명시된 건 3건(financial-manager·training-coach·this-site). 지어내지 않고 사실대로 렌더한다 |
| 2026-09-22 | `prefers-reduced-motion` 을 fail-open → **fail-closed** 로 | app/globals.css | `(reduce){끄기}` 구조는 OS가 선호를 보고하지 못하면 조용히 애니메이션이 켜진다. `(no-preference){켜기}` 로 뒤집었다. Windows 는 opt-in 비율이 거의 0이라 "reduce 가 적다=모션을 원한다"로 읽으면 안 된다 |
| 2026-09-22 | `.gitignore` 의 `*token*` 이 `lib/tokens.ts` 를 먹고 있었다 | .gitignore | 비밀 차단 규칙이 디자인 토큰 모듈을 잡아 **한 번도 커밋되지 않았고**, `app/icon.tsx` 가 import 하므로 **클론하면 빌드가 실패**했다. 로컬에는 파일이 있어 영원히 재현되지 않는다. 클론 시뮬레이션으로 확인 후 `!lib/tokens.ts` 예외 추가 |
| 2026-09-22 | 공개 스킬 파일에서 교회 식별 정보·민감 보유 목록 제거 | skills/site-copy, skills/publish-gate | 게이트 예시에 `"KCPC, a Korean church in Atlanta"` 가 박혀 있었고, 같은 저장소의 "청년부 내부 앱" 카드와 합쳐지면 어느 교회의 성도 시스템인지 특정된다. 게이트 문서가 "무엇을 감추는지"를 공개하던 것도 일반화. 히스토리는 사용자 결정으로 재작성하지 않는다 |
